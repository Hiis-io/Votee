...
sealed trait BaldWin[C <: Candidate, B[+CC >: C <: Candidate] <: Ballot[CC, B]]
  ...
  final override def run(ballots: Seq[B[C]], candidates: Seq[C], vacancies: Int)(using
    ...
    if candidates.length == 1 then bordaScores(ballots, candidates).toList.map(Winner(_))
    else
      // removing the lowest borda score candidate from the candidate list
      val lowestBordaCandidate = resolveTies(
        bordaScores(ballots, candidates).toList.sortWith(_._2 < _._2)
      ).head
...
case object BaldWin:
  def run[C <: Candidate, B[+CC >: C <: Candidate] <: Ballot[CC, B]](
    ...
    new BaldWin[C, B] {}.run(ballots, candidates, vacancies)
end BaldWin
