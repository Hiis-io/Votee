...
/**
 * Ballot represents the base contract of an election ballot implementation.
 * User could define their own ballot base on this contract to suite their needs.
 * 
 * @param id the ballot id
 * @param weight the ballot weight
 * @param preferences the ballot preferences
 *                    
 * @tparam C The ballot candidate type
 * @tparam T The ballot concrete type. Required for some utility methods
 * @see [[PreferentialBallot]]
 */
trait Ballot[+C <: Candidate, +T[+CC >: C <: Candidate] <: Ballot[CC, T]](
   ...
   *
   * @param candidates the candidates to excludes
   * @tparam CC candidates type
   * @return new ballot with some excluded candidates
   */
  @targetName("exclude")
  def --[CC >: C <: Candidate](candidates: Seq[CC]): T[CC]

  /**
   * Includes the desired candidates found in the list from the ballot
   * Note: these candidates are assumed by the algorithms to be appended to the candidates list.
   *
   * @param candidates the candidates too be included
   * @tparam CC candidates type
   * @return new ballot with some included candidates
   */
