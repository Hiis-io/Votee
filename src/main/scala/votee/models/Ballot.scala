...
trait BallotOps[C <: Candidate]:
  ...
  def includeCandidates(candidates: List[C]): BB
  def excludeCandidates(candidates: List[C]): BB
end BallotOps
...
final case class PreferentialBallot[C <: Candidate](override val id: Int, override val weight: Rational = Rational(1, 1), override val preferences: List[C])
  ...
  override def excludeCandidates(candidates: List[C]): BB = PreferentialBallot(id, weight, preferences.filterNot(candidates.contains(_)))
